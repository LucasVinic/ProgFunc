...
evalP (Prog fs) = result where
  result = eval ([], (updatecF [] fs)) (Call (Ident "main") [])
...
eval context x = case x of
  EAdd expL expR -> value where
    valueL = eval context expL 
    valueR = eval context expR
    value = valueL + valueR
    newContext = context
  ...
  Call funcId paramExps -> value where
    ...
    value = eval (paramBindings, oldFContext) funcExp 
  EIf eCond eThen eElse -> value where
    condVal = eval context eCond
    value = if (condVal /= 0)
      then (eval context eThen)
      else (eval context eElse)
